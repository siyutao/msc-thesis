...
Four experiments are then presented: Experiment 2 (§\ref{sec:exp2-verb-specificity}) defines entropy measures of variability of individual dependencies and seeks to quantitatively verify the verb specificity test that is used to distinguish core and non-core dependents (complements and adjuncts); Experiment 3 (§\ref{sec:exp3-valency-frame-entropy}) uses joint entropy to define the valency frame entropy measure and tests its correlation with verb frequency; Experiment 4 (§\ref{sec:exp4-ablation}) examines cross-lingual variations in how languages encode the valency frames, by means of an ablation study using conditional entropy to assess the contribution of word order and case marking to the overall valency frame entropy; Experiment 5 (§\ref{sec:exp5-verb-entropy}) takes a frame-based approach and, in symmetry to Experiment 3, calculates the correlation between verb entropy and frame frequency.
...
\section{Experiment 2: Verb specificity between core and non-core dependents}\label{sec:exp2-verb-specificity}

Verb specificity \citep{haspelmath2015a} 
any non-specific dependent relations would become part of the baseline
however the verb specificity 
...
Recall the bimodal distribution of transitive verbs, verbs that are highly transitive or highly intransitive will have high entropy wrt object, carrier more information. Verb specific feature leads to higher entropy.
- IF core vs non-core distinction holds, we'll see higher entropy values for core dependencies than for non-core. Because core dependencies are more verb specific. 
...
\section{Experiment 3: Frequency correlation with valency frame entropy}\label{sec:exp3-valency-frame-entropy}
...
\section{Experiment 4: Word order or case? Cross-lingual variation in valency encoding strategies}\label{sec:exp4-ablation}
